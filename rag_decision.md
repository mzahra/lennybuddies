...
**Why we call this non-RAG, not RAG:** TF-IDF/cosine similarity is a classical, sparse, local relevance-scoring technique — not the dense-embedding, vector-database retrieval most people mean by "RAG" in practice. We treat it as a lightweight pre-processing step within a non-RAG context-injection design, not a RAG pipeline.
